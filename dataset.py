...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        end_input_tokens = self.tokenizer_src.encode(src_text).ids
        dec_input_tokens = self.tokenizer_tgt.encode(tgt_text).ids

        enc_num_padding_tokens = self.seq_len - len(end_input_tokens) - 2
        ...
        encoder_input = torch.cat(
            [self.sos_token,
             torch.tensor(end_input_tokens, dtype=torch.int64),
             self.eos_token,
